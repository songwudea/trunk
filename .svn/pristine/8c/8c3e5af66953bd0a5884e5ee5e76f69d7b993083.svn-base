    	...
    	$('#tt').datagrid({  
    		...
    		toolbar:[{
	        	text:'新增',  
	         	...
	         	plain:true,
	         	handler:function() {
	         		document.location.href="${ctx}/admin/content/edit.html?parentId=${param.parentId}";
	         	}
     		 },{  
	        	text:'修改',  
	         	plain:true,
			...
			width:"820",
			method:"post",
			url:'${ctx}/admin/content/list2.html?parentId=${param.parentId}',  
			pagination:true,
			striped:true,
			rownumbers:true,
			sortName:'id',
    		sortOrder:'asc',
	     	columns:[[  
	        	...
	        	{field:'urlRewrite',title:'url重写',width:120, sortable:true},
	        	{field:'hitCounter',title:'点击数',width:60, sortable:true}
	    	]]  
		}); 
