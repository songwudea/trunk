...
(function () {
    ...
    var tmp = window.location.pathname, URL= "../../common/js/ueditor/";
//         URL= tmp.substr(0,tmp.lastIndexOf("\/")+1).replace("_examples/","");//这里你可以配置成ueditor目录在您网站的相对路径或者绝对路径（指以http开头的绝对路径）
       
    UEDITOR_CONFIG = {
        imagePath:"/img/", //图片文件夹所在的路径，用于显示时修正后台返回的图片url！具体图片保存路径需要在后台设置。！important
...
//            var block = X(inline, {p:1,div:1,blockquote:1,$:{style:1,dir:1}});
            return {
                ...
                blackList:{style:1,  link:1, object:1, applet:1, meta:1, base:1, '#comment':1}
//                whiteList: {
