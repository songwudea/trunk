...
<div  id="menu"><h2>产品系列</h2>
        ...
        <c:forEach var="item" items="${categoryList}" varStatus="state">
          <li><a href="${ctx}/eyes/${item.path}.html" class="current"><b>${item.name}测试</b></a></li>
        </c:forEach>
        ...
        <c:forEach var="item" items="${itemList}" varStatus="state">
          <div class="box">
            <div class="box_tit"> <span class="chn"><a href="${ctx}/product/p${item.id}.html">${item.name}112</a></span><br />
              <span class="eng">- -  ${item.itemTitle}</span> </div>
            <div class="box_content"> 
	            <a href="${ctx}/product/p${item.id}.html">
	            <img src="${ctx}/img/${item.itemImage[0].url}" alt="${item.name}测试1"/>
	            </a>
              <div class="describe">
